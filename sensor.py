...
from homeassistant.components.sensor import PLATFORM_SCHEMA, ENTITY_ID_FORMAT, SensorDeviceClass, SensorStateClass
...
class DataLoader():
    ...
    def getUVIndexes(self):
        ...
        try:
            ...
            _LOGGER.error("getUVIndexes failed to fetch data: %s failed with %s", self._radResource, ex)
            self.headers = None
    ...
    def getTemperatures(self):
        ...
        try:
            ...
            _LOGGER.error("getTemperatures failed to fetch data: %s failed with %s", self._tempResource, ex)
            self.headers = None
...
ISRAELBEACHES = ['Northern Coast','Central Coast','Southern Coast', 'Sea of Galilee','Gulf of Elat']
